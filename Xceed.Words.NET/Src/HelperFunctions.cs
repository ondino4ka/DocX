...
namespace Xceed.Words.NET
{
  internal static class HelperFunctions
  {
    ...
    /// <summary>
    /// If this document does not contain a /docProps/app.xml add the default one generated by Microsoft Word.
    /// </summary>
    /// <param name="document"></param>
    /// <returns></returns>
    internal static void CreateAppPropertiesPart(DocX document)
    {
      var appPropertiesPart = document._package.CreatePart(new Uri("/docProps/app.xml", UriKind.Relative), "application/vnd.openxmlformats-officedocument.extended-properties+xml", CompressionOption.Maximum);
      var appPropDoc = new XDocument
      (
       new XDeclaration("1.0", "UTF-8", "yes"),
       new XElement
       (
      ...
      using (TextWriter tw = new StreamWriter(new PackagePartStream(appPropertiesPart.GetStream(FileMode.Create, FileAccess.Write))))
      {
        appPropDoc.Save(tw, SaveOptions.None);
      }
      
      document._package.CreateRelationship(appPropertiesPart.Uri, TargetMode.Internal, "http://schemas.openxmlformats.org/officeDocument/2006/relationships/extended-properties");
    }
